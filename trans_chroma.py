...
parser.add_argument('-c','--color',dest="color",default="ffffff",help="Hexadecimal color of reference rrggbb")
parser.add_argument('-s','--sensitivity',dest="sensitivity",type=float,default=.05,help="sensitivity of the chroma key (stdev)")
...
if args.simulate:
	print "Simulate"
	cmdline='ffplay -hide_banner  -f lavfi "movie='+ pipes.quote(args.path)+cropStr+",split=2[f][b];[b]chromakey=0x"+args.color+":%3f"%args.sensitivity+" [trans];[f]lutrgb=r=0:b=0:g=0[black];[black][trans]overlay, blackdetect=d=%3.f"%args.duration+":pix_th=%.2f"%args.threshold+'" '
	print cmdline
	try:
...
cmdline='ffprobe -hide_banner  -f lavfi "movie='+ pipes.quote(args.path)+cropStr+",split=2[f][b];[b]chromakey=0x"+args.color+":%3f"%args.sensitivity+" [trans];[f]lutrgb=r=0:b=0:g=0[black];[black][trans]overlay, blackdetect=d=%3.f"%args.duration+":pix_th=%.2f"%args.threshold+',select=gt(pts-prev_selected_pts\,1)" '
print cmdline
try:
