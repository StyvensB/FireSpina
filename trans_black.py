...
parser.add_argument('--chroma',dest="chroma",help="color of the chroma key")
...
if args.chroma is not None:
	chromaStr= ",chromakey=0x"+args.chroma+":.05:0.0,alphaextract"
	print chromaStr
else:
	chromaStr=""
...
cmdline='ffprobe -hide_banner  -f lavfi "movie='+ pipes.quote(args.path)+cropStr+chromaStr+",eq=brightness=-.1,blackdetect=d=%3.f"%args.duration+":pix_th=%.2f"%args.threshold+':pic_th=0.99,select=gt(pts-prev_selected_pts\,1)" '
print cmdline
try:
